...
const Trips = ({ userInfo }) => {
  ...
  useEffect(() => {
    const fare =
      busRouteList.find((route) => route._id === trips.routeId)?.totalFare || 0;
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedDepartureTime = moment(trips.departureTime)
      .tz("Asia/Ho_Chi_Minh")
      .format("YYYY-MM-DDTHH:mm");
    const formattedEndTime = moment(trips.endTime)
      .tz("Asia/Ho_Chi_Minh")
      .format("YYYY-MM-DDTHH:mm");
    ...
    try {
      ...
      if (editingTripsId) {
        response = await fetch(
          `${API_URL}/api/tripsRoutes/update/${editingTripsId}`,
          {
            method: "PUT",
            headers: {
  ...
  const fetchTripsList = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/tripsRoutes/list?userId=${userInfo._id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchBusList = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/buses/list?userId=${userInfo._id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchBusRouteList = async () => {
    try {
      const response = await fetch(
        `${API_URL}/api/busRoutes/list?userId=${userInfo._id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const onEdit = (tripsItem) => {
    const departureTimeEdit = moment(
      tripsItem.departureTime,
      "DD/MM/YYYY, HH:mm"
    )
      .tz("Asia/Ho_Chi_Minh")
      .format("YYYY-MM-DDTHH:mm");
    const endTimeEdit = moment(tripsItem.endTime, "DD/MM/YYYY, HH:mm")
      .tz("Asia/Ho_Chi_Minh")
      .format("YYYY-MM-DDTHH:mm");
  ...
  const onDelete = async (tripsItem) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(
          `${API_URL}/api/tripsRoutes/delete/${tripsItem._id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
                    ...
                    <MenuItem key={busRoute._id} value={busRoute._id}>
                      <Box sx={{ display: "flex", alignItems: "center" }}>
                        ...
                        <Typography sx={{ fontSize: "10px" }}>
                          {busRoute.departure} đến {busRoute.destination}
                        </Typography>
                        )
                      </Box>
                    </MenuItem>
                ...
                .map((tripsItem, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell sx={{ textAlign: "center", fontSize: "13px" }}>
                      {
                        busRouteList.find(
                          (route) => route._id === tripsItem.routeId
                        )?.routeName
                      }
                    </TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "13px" }}>
                      {
                        busList.find((bus) => bus._id === tripsItem.busId)
                          ?.busType
                      }
                    </TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "13px" }}>
                      {tripsItem.bookedSeats}/
                      {
                        busList.find((bus) => bus._id === tripsItem.busId)
                          ?.cartSeat
                      }{" "}
                    ...
                    <TableCell sx={{ textAlign: "center", fontSize: "13px" }}>
                      {tripsItem.totalFareAndPrice} VND
                    </TableCell>
                    <TableCell sx={{ textAlign: "center", fontSize: "13px" }}>
                      {tripsItem.tripType}
                    </TableCell>
